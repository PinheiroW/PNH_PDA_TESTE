// Ficheiro: PNH_Tablet/Scripts/5_Mission/PNH_Tablet/App_Map/Components/PNH_MapEdit.c

class PNH_MapEdit extends ScriptedWidgetEventHandler {

    PNH_MapUI m_Menu; // Atualizado para PNH_MapUI
    ref PNH_MarkerData m_Marker; // Atualizado para PNH_MarkerData
    int m_MarkerIndex;
    ...
    ref PNH_IconListAdapter m_IconListAdapter;
    ref PNH_ColorListAdapter m_ColorListAdapter;

    void PNH_MapEdit(PNH_MapUI menu, Widget root) {
        m_Menu = menu;
        ...
        m_IconListAdapter = new PNH_IconListAdapter(m_PanelIcon);
        m_ColorListAdapter = new PNH_ColorListAdapter(this, m_PanelColor);

        m_Root.SetHandler(this);
    }

    void SetMarker(int markerIndex, PNH_MarkerData marker, bool isCustomServer, bool isNewMarker, string markerSuffix) {
    ...
    override bool OnMouseButtonDown( Widget w, int x, int y, int button ) {
        if (button == MouseState.LEFT) {
            if (w == m_PanelClose || w == m_ButtonClose) {
                ...
                m_Menu.EditMarkerAttributes(m_MarkerIndex, name, m_MarkerSuffix, iconPath, color, m_IsCustomServerMarker, false);
                m_Menu.HideDialog();
                return true;
